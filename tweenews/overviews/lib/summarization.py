...
news_weight = [0.375, 1.0, -1, 1.0] #for content relevance, time relevance, content similarity, time diversity
# news_threshold = 0.005
news_threshold_damping_ratio = 0.35  #0.35
tweets_threshold = 0.5
tweets_weight = [1, 1.0, 0.5, -1] #for content relevance, sentiment, content similarity with news (pos or neg), and content similarity
beta = 1.0
cohits_mu = 10.0
...
def computeBM25(news_vec, tweets_vec, word_distribution):
    ...
    for i in xrange(len(top_words)):
        ...
        news_score += dist[i] * ((2.6 * top_news_vec[i, :]) / (top_news_vec[i, :] + 1.6 * (0.25 + 0.75 / avg_doc_len_n * news_len )))

        # tweets_freq = np.ceil(top_tweets_vec[i, :])

        tweets_score += dist[i]  * ((2.6 * top_tweets_vec[i, :]) / (top_tweets_vec[i, :] + 1.6 * (0.25 + 0.75 / avg_doc_len_t * tweets_len )))
...
def getNewsSummary(news, news_rele, news_titles_vec, mu, sigma, topk):
    ...
    for i in xrange(0, topk):
        ...
        print
        div = updateDiversity(news_titles_vec[selected, :], i, news_titles_vec, div)
...
def getTweetsSummary(sentiCL, tweets, tweets_rele, tweets_content_vec, news_summary_vec_list, topk):
    ...
    for i in xrange(len(news_summary_vec_list)):
        news_sim = updateTweetsDiversity(news_summary_vec_list[i], i, tweets_content_vec, news_sim)
    ...
    for i in xrange(0, topk):
        ...
        print 
        div = updateTweetsDiversity(tweets_content_vec[selected, :], i, tweets_content_vec, div)

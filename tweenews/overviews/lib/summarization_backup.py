...
#note the relevance score of BM25 is about 3-4 times smaller than cosine, so the first weight need to be larger.
news_weight = [3, 1.0, -1, 1.0] #for content relevance, time relevance, content similarity, time diversity
# news_threshold = 0.005
news_threshold_damping_ratio = 0.35  #0.35
tweets_threshold = 0.5
tweets_weight = [4.0, 1.0, 0.06, -2] #for content relevance, sentiment, content similarity with news (pos or neg), and content similarity
beta = 1.0
cohits_mu = 10.0
...
def summarization(sentiCL, news, tweets, word_distribution, vectorizer, mu, sigma, topk, debug = 0):
    ...
    news_rele, tweets_rele = computeBM25(news_titles_vec, tweets_content_vec, word_distribution)
    print time.time()-t2
    # news_rele, _ = computeContentRelevance(news_titles_vec, tweets_content_vec, word_distribution)
    tweets, tweets_content_vec, tweets_rele = getRelevantTweets(tweets, tweets_content_vec, tweets_rele)
    t3 = time.time()
    print 'cosine', (t3 - t2)
    ...
    news_summary, news_summary_vec = getNewsSummary(news, news_rele, news_titles_vec, mu, sigma, topk)
    t5 = time.time()
    print 'news summary time', (t5 - t4)
    # print '======================'

    print "t5-t6 tweets length", len(tweets)
    tweets_summary, sentiment = getTweetsSummary(sentiCL, tweets, tweets_rele, tweets_content_vec, news_summary_vec, topk)
    t6 = time.time()
    print 'tweets summary time', (t6 - t5)

    #news_summary is a list of news objects, and the tweets_summary is a list of tweet text.
    print 
    print
...
def computeBM25(news_vec, tweets_vec, word_distribution):
    ...
    avg_doc_len = float(news_vec.tocoo().row.size + tweets_vec.tocoo().row.size) / (news_vec.shape[0] + tweets_vec.shape[0])

    top_news_vec = news_vec[:, top_words].T
    top_tweets_vec = tweets_vec[:, top_words].T

    # news_words = top_news_vec.tocoo().col
    # tweets_words = top_tweets_vec.tocoo().col

    news_words_freq = np.bincount(top_news_vec.tocoo().row)
    tweets_words_freq = np.bincount(top_tweets_vec.tocoo().row)

    if news_words_freq.size < top_count:
        news_words_freq = np.append(news_words_freq, np.zeros(top_count - news_words_freq.size))
    if tweets_words_freq.size < top_count:
        tweets_words_freq = np.append(tweets_words_freq, np.zeros(top_count - tweets_words_freq.size))

    idf = news_words_freq + tweets_words_freq
    idf = np.log((news_vec.shape[0] + tweets_vec.shape[0] + 0.5 - idf) / (idf + 0.5))
    ...
    for i in xrange(len(top_words)):
        ...
        news_score += dist[i] * ((2.6 * top_news_vec[i, :]) / (top_news_vec[i, :] + 1.6 * (0.25 + 0.75 / avg_doc_len * news_len )))

        tweets_freq = np.ceil(top_tweets_vec[i, :])

        tweets_score += dist[i]  * ((2.6 * top_tweets_vec[i, :]) / (top_tweets_vec[i, :] + 1.6 * (0.25 + 0.75 / avg_doc_len * tweets_len )))
...
def computeContentRelevance(news_vec, tweets_vec, word_distribution):
    ...
    for i in xrange(news_vec.shape[0]):
        news_score.append(computeCosSim(news_vec[i, :].toarray()[0], word_distribution, dist_len))

    tweets_score = []
    for i in xrange(tweets_vec.shape[0]):
        tweets_score.append(computeCosSim(tweets_vec[i, :].toarray()[0], word_distribution, dist_len))
...
def getNewsSummary(news, news_rele, news_titles_vec, mu, sigma, topk):
    ...
    for i in xrange(0, topk):
        updated_score = score + news_weight[2] * div + news_weight[3] * time_div
        ...
        if updated_score[selected] < news_threshold:
            break
        if news_threshold == 0:
            news_threshold = updated_score[selected]* news_threshold_damping_ratio
        ...
        # print
        updateDiversity(news_titles_vec[selected, :], i, news_titles_vec, div)
        updateTimeDiversity(news[selected].created_at, news, time_div, mu, sigma)
...
def getTweetsSummary(sentiCL, tweets, tweets_rele, tweets_content_vec, news_summary_vec_list, topk):
    ...
    for i in xrange(len(news_summary_vec_list)):
        updateTweetsDiversity(news_summary_vec_list[i], i, tweets_content_vec, news_sim)
    ...
    for i in xrange(0, topk):
        ...
        elif tweets_rele[selected]< 0.5*highest_tweets_rele:
            break
        ...
        # print 
        updateTweetsDiversity(tweets_content_vec[selected, :], i, tweets_content_vec, div)
...
def coHits(news, tweets, news_vec, tweets_vec, news_rele, tweets_rele):
    ...
    full_network = np.concatenate((network.dot(network.T), beta * network), axis = 1)
    full_network = np.concatenate((full_network, np.concatenate((beta * network.T, network.T.dot(network)), axis = 1)), axis = 0)

    score = np.concatenate((news_rele, tweets_rele), axis = 1).T
    
    score = cohits_mu / (1.0 + cohits_mu) * np.linalg.inv((np.eye(score.size) - 1.0 / (1.0 + cohits_mu) * diag * full_network * diag)).dot(score)
...
def getNetwork(news_vec, tweets_vec):
    ...
    for i in xrange(news_vec.shape[0]):
        ...
        for j in xrange(tweets_vec.shape[0]):
            row.append(computeCosSim(tweets_vec[j, :].toarray()[0], n_vec, 1.0))
        network.append(row)
...
def getDiagonalMat(network):
    ...
    diag_values = np.concatenate((news_diag_values[news_nonzeros], tweets_diag_values[tweets_nonzeros]), axis = 1)
